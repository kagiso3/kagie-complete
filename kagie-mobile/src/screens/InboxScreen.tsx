  ...
  return (
    <ScreenScroll>
        ...
        {notifications.length ? (
          notifications.map((item) => (
            <View key={item.id} style={[styles.notificationRow, item.read && styles.notificationRead]}>
              ...
              {!item.read ? (
                <Button label="Mark read" onPress={() => {
      ...
      <Card>
        ...
        {(support?.messages || []).map((item) => (
          <View key={item.id} style={[styles.messageBubble, item.senderRole === "user" ? styles.messageUser : styles.messageAssistant]}>
            <Text style={styles.messageRole}>{item.senderRole === "user" ? "You" : "Kagie assistant"}</Text>
            <Text style={styles.messageText}>{item.message}</Text>
          </View>
        ))}
        <Field label="Message" value={message} onChangeText={setMessage} multiline />
        <Button label={syncing ? "Sending..." : "Send support message"} onPress={handleSend} />
      </Card>
      ...
      <Card>
        ...
        <Field label="What should the assistant prepare?" value={callback.note} onChangeText={(value) => setCallback((current) => ({ ...current, note: value }))} multiline />
        <Button label={syncing ? "Sending..." : "Request callback"} onPress={handleCallback} tone="secondary" />
      </Card>
    </ScreenScroll>
  );
}
