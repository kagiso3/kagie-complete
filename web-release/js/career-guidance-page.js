(function () {
  ...
  async function main() {
    ...
    let paperUploading = false;
    async function handleUploadSubmit(event) {
      event.preventDefault();
      if (paperUploading) return;
      const file = $("uploadFile")?.files?.[0] || null;
      const submitButton = event.submitter || $("paperUploadForm")?.querySelector('button[type="submit"]');
      ...
      try {
        ...
        setUploadMessage("Uploading past paper...", "success");
