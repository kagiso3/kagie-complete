(function () {
  ...
  async function runFormAction(event, busyText, task) {
    ...
    const slowTimer = window.setTimeout(() => {
      if (form.dataset.busy === "1" && statusNode) {
        statusNode.className = "status-message info";
        statusNode.textContent = "Still processing, please wait...";
      }
    }, 3200);
    if (button) {
  ...
  async function runButtonAction(button, key, busyText, task) {
    const lockKey = String(key || button?.dataset?.action || button?.textContent || "button-action");
