(function () {
  ...
  async function runButtonAction(button, key, busyText, task) {
    const lockKey = String(key || button?.dataset?.action || button?.id || "button-action");
  ...
  function validatePaperFile(file) {
    ...
    if (Number(file.size || 0) > MAX_PAPER_BYTES) {
      return "Keep the question paper under 20 MB so the upload stays fast.";
    }
    return "";
  }
  ...
  async function savePaper() {
    ...
    const fileError = validatePaperFile(file);
    if (fileError) {
      clearFormMessage("err", fileError);
      return;
    }
    const payload = {
    ...
    try {
      slowTimer = window.setTimeout(() => {
        clearFormMessage("info", "Still uploading, please wait...");
      }, 3200);
      if (state.editingId) {
  ...
  function bindEvents() {
    ...
    $("questionPapersRefreshBtn")?.addEventListener("click", (event) => {
      runButtonAction(event.currentTarget, "question-papers-refresh", "Refreshing...", refresh);
    });
    $("questionPapersSignOut")?.addEventListener("click", () => state.api.logoutReal?.() || state.api.logout?.());
    $("questionPaperSaveBtn")?.addEventListener("click", (event) => {
      runButtonAction(event.currentTarget, "question-paper-save", "Saving...", savePaper);
    });
    $("questionPaperRows")?.addEventListener("click", (event) => {
      ...
      if (button.dataset.action === "publish") runButtonAction(button, `paper-publish:${paperId}`, "Publishing...", () => updateStatus(paperId, "Published"));
      if (button.dataset.action === "disable") runButtonAction(button, `paper-disable:${paperId}`, "Disabling...", () => updateStatus(paperId, "Disabled"));
      if (button.dataset.action === "delete") runButtonAction(button, `paper-delete:${paperId}`, "Deleting...", () => deletePaper(paperId));
    });
