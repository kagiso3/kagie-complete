(function () {
  ...
  async function main() {
    ...
    $("sendBtn").addEventListener("click", async () => {
      ...
      try {
        ...
        slowTimer = window.setTimeout(() => {
          showSoftSupportStatus("Still processing, please wait...");
        }, 3200);
        ...
        if (!isRecoverable(error)) {
          showSoftSupportStatus(error?.message || "Could not send your message right now.");
          return;
        }
    ...
    $("callbackBtn").addEventListener("click", async () => {
      ...
      let slowTimer = 0;
      try {
        requestingCallback = true;
        setButtonBusy($("callbackBtn"), "Submitting...", true);
        slowTimer = window.setTimeout(() => {
          showSoftSupportStatus("Still processing, please wait...");
        }, 3200);
        ...
        if (!isRecoverable(error)) {
          showSoftSupportStatus(error?.message || "Could not save the callback request right now.");
          return;
        }
        ...
        setButtonBusy($("callbackBtn"), "", false);
      }
      $("callbackTime").value = "";
