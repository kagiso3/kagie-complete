  ...
  async function main() {
    ...
    if (isStaff) {
      ...
      $("uploadForm").addEventListener("submit", async (event) => {
        ...
        if (prospectusUploading) return;
        const file = $("uploadFile").files?.[0] || null;
        const submitButton = event.submitter || $("uploadForm").querySelector('button[type="submit"]');
        const originalText = submitButton?.textContent || "Upload PDF";
        let slowTimer = 0;
        setUploadMessage("");
        try {
          prospectusUploading = true;
          if (submitButton) {
          ...
          slowTimer = window.setTimeout(() => {
            setUploadMessage("Still uploading, please wait...", "success");
          }, 3200);
