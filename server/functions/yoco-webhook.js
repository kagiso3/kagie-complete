...
exports.handler = async (event) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !webhookSecret) {
    return json(500, { message: "Yoco webhook configuration is missing on the server." }, origin);
  }

  const rawBody = readRawBody(event);
  const verification = verifyYocoWebhook(event.headers || {}, rawBody, webhookSecret);
  if (!verification.ok) {
  ...
  try {
    ...
    return json(400, { message: "Yoco webhook body must be valid JSON." }, origin);
  }
  ...
  try {
    ...
    const appRows = await supabaseFetch(
      supabaseUrl,
      `/rest/v1/applications?id=eq.${encodeFilter(paymentRow.application_id)}&select=id,user_id,status,payment_status,submitted_at,payment_note,assigned_assistant_id&limit=1`,
      {
        method: "GET",
        headers: adminHeaders(serviceRoleKey)
      }
    ...
    const nextMeta = {
      ...
      gatewayStatus,
      reviewedAt: nowISO(),
      verifiedAt: eventType === "payment.succeeded" ? nowISO() : currentMeta.verifiedAt
    };
    ...
    if (eventType === "payment.succeeded") {
      ...
      await bestEffortStaffAlerts(
        ...
        `A learner Yoco payment was verified${paymentRow.reference ? ` for reference ${paymentRow.reference}` : ""}. Open Kagie to continue the application flow.`,
        "success"
      );
      ...
      await bestEffortNotification(
        ...
        serviceRoleKey,
        application.user_id,
        "Yoco payment not completed",
        "Your Yoco payment did not complete. Your cart is still available so you can retry payment.",
        "warning"
      );
      await bestEffortStaffAlerts(
        supabaseUrl,
        ...
        `A learner Yoco payment did not complete${paymentRow.reference ? ` for reference ${paymentRow.reference}` : ""}. Check the learner payment lane if follow-up is needed.`,
        "warning"
      );
    }
    ...
    return json(typeof error?.status === "number" ? error.status : 500, {
      message: error?.message || "Could not process Yoco webhook."
    }, origin);
  }
};
